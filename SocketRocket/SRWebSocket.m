...
- (void)didConnect;
{
    ...
    __unused int retval = SecRandomCopyBytes(kSecRandomDefault, keyBytes.length, keyBytes.mutableBytes);
...
- (void)_sendFrameWithOpcode:(SROpCode)opcode data:(id)data;
{
    ...
    if (!useMask) {
        ...
        __unused int retval = SecRandomCopyBytes(kSecRandomDefault, sizeof(uint32_t), (uint8_t *)mask_key);
